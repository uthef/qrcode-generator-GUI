...
from fsutils import QR_CODES_DIR, TEMP_DIR, make_sure_directory_exists

class MainWindow:
    def __init__(self, app):
        self.app = app
        self.layout = MainWindowLayout(self)
        make_sure_directory_exists(TEMP_DIR)
        self.layout.build()
        self.update_ui()
    ...
    def create_qr(self, mode=""):
        ...
        if mode == "temp": # If mode == "temp", save image to temp folder for use in the preview window
            ...
            if self.layout.logo_checkbox.get() == 1: # generates qr code and adds logo if enabled
                ...
                logo = Image.open(f"{TEMP_DIR}/logo.png").convert("RGBA")
    ...
    def update_ui(self, _ = ""):
        ...
        if self.layout.preview_checkbox.get() == 1:
            ...
            self.create_qr("temp")
            self.layout.preview_image.configure(image= tkinter.PhotoImage(file=f"{TEMP_DIR}/temp.png"))
